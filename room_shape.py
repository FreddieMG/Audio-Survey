...
def main():
    ...
    if not st.session_state.room_shape_completed and st.session_state.room_shape_current_audio_index < total_audios:
        ...
        with st.form(key="transcription_form"):
            ...
            existing_transcription = st.session_state.room_shape_transcriptions.get(parameter, "")
            transcription = st.text_area(
                "Enter transcription for the above audio:",
                value=existing_transcription,
                key=f"room_shape_transcription_{st.session_state.user_id,}_{audio_number}",
            )
        ...
        if submit_button:
            if not transcription.strip():
                ...
                st.session_state.room_shape_transcriptions[parameter] = transcription
        ...
        with col1:
            if st.button("Commit and Save"):
                ...
                st.session_state.current_page = "Rotation Speed"
                st.rerun()

        with col2:
            if not st.session_state.room_shape_completed:
                if st.button("Go Back"):
